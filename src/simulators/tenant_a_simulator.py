"""
IoT Simulator for Tenant A (Bogotá Water Network)
Generates synthetic sensor data for 50 pipe segments
"""
...
class TenantASimulator:
    """
    Simulates IoT sensor readings for Bogotá water network
    50 pipe segments with pressure, flow, and vibration sensors
    """
    ...
    def __init__(self, num_segments: int = 50):
        ...
        self.city = "Bogotá"
        self.altitude = 2600  # meters above sea level
        
        # Base parameters for Bogotá (high altitude = lower atmospheric pressure)
        self.base_pressure = 4.5  # bar (typical for Bogotá)
        self.base_flow = 0.8  # m³/s
        self.base_vibration = 2.5  # mm/s
        
        # Sensor noise characteristics
        self.pressure_std = 0.15
        self.flow_std = 0.05
        self.vibration_std = 0.3
    ...
    def simulate_batch(self, duration_minutes: int = 5, frequency_hz: float = 1.0) -> List[Dict]:
        ...
        active_anomalies = {}  # segment_id -> end_time
        ...
        for i in range(total_readings):
            ...
            for segment_idx in range(self.num_segments):
                ...
                if segment_id not in active_anomalies and random.random() < self.anomaly_rate:
                    # Start new anomaly
                    active_anomalies[segment_id] = timestamp + timedelta(seconds=self.anomaly_duration)
                
                # Check if anomaly is still active
                is_anomaly_active = segment_id in active_anomalies and timestamp < active_anomalies[segment_id]
                ...
                if is_anomaly_active:
                    # Apply anomaly
                    anomaly_type = random.choice(["pressure_drop", "flow_anomaly"])
                    reading = self._generate_anomaly_reading(normal_reading, anomaly_type)
                else:
                    reading = normal_reading
                    # Clean up expired anomalies
                    if segment_id in active_anomalies and timestamp >= active_anomalies[segment_id]:
...
def main():
    ...
    simulator = TenantASimulator(num_segments=50)
