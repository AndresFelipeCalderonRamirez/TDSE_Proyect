"""
IoT Simulator for Water Twin Prototype
Multi-tenant simulation for Bogotá and Medellín water networks
"""
...
class IoTSimulator:
    """
    Multi-tenant IoT simulator for water networks
    Generates sensor data and publishes to Kinesis
    """
    ...
    def __init__(self, tenant_id: str, config: Dict):
        ...
        # AWS Kinesis client
        self.kinesis = boto3.client('kinesis', region_name=os.getenv('AWS_DEFAULT_REGION', 'us-east-1'))
    ...
    def _publish_to_kinesis(self, readings: List[SensorReading]) -> bool:
        """Publish readings to Kinesis stream"""
        try:
            ...
            for reading in readings:
                record = {
                    ...
                    'PartitionKey': reading.tenant_id  # Partition by tenant
                }
            ...
            logger.error(f"Failed to publish to Kinesis: {str(e)}")
            return False
    
    def _export_ground_truth(self, filename: str = "anomaly_ground_truth.csv"):
    ...
    def simulate(self, duration_seconds: int = 300, export_ground_truth: bool = False) -> bool:
        ...
        Returns:
            Success status
        """
        logger.info(f"Starting simulation for {self.city} ({self.tenant_id})")
        ...
        # Publish to Kinesis
        success = self._publish_to_kinesis(readings)
        
        # Export ground truth if requested
        if export_ground_truth:
            filename = f"anomaly_ground_truth_{self.tenant_id}.csv"
            self._export_ground_truth(filename)
...
def run_tenant_simulation(tenant_id: str, duration: int, export_ground_truth: bool) -> bool:
    ...
    simulator = IoTSimulator(tenant_id, config[tenant_id])
    return simulator.simulate(duration, export_ground_truth)

def main():
    """Main function with CLI interface"""
    parser = argparse.ArgumentParser(description='Water Twin IoT Simulator')
    ...
    logger.info("Water Twin IoT Simulator")
